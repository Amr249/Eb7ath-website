...
function resolveSchedule(input) {
  ...
  const scheduledAt = toDateOrNull(input.scheduledAt || input.publishedAt);
  const now = new Date();
  ...
  if (requested === "scheduled") {
    ...
    if (scheduledAt <= now) {
      return { status: "published", publishedAt: scheduledAt.toISOString() };
    }
    return { status: "scheduled", publishedAt: scheduledAt.toISOString() };
  }
...
function mapPublicArticle(row, locale) {
  return {
    id: row.id,
    slug: row.slug,
...
/** Promote due scheduled articles to published (idempotent). */
...
export async function listPublishedArticles(locale, limit) {
  ...
  const rows = limit
    ...
    : await sql()`
        ...
        ORDER BY a.published_at DESC NULLS LAST, a.updated_at DESC
      `;
  return rows.map((row) => mapPublicArticle(row, locale));
}
...
export async function getPublishedArticleBySlug(slug, locale) {
  ...
  if (rows.length) {
    return mapPublicArticle(rows[0], locale);
  }
...
export async function createArticle(input) {
  ...
  const { status, publishedAt } = resolveSchedule(input);
