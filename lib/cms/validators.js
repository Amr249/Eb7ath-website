...
const institutionExpertLocalizedSchema = z.object({
  name: z.string().min(1),
  affiliation: z.string().min(1),
});

export const institutionExpertInputSchema = z.object({
  slug: z.string().optional(),
  researchgateUrl: z.string().url().min(1),
  status: z.enum(["draft", "published"]).default("draft"),
  sortOrder: z.coerce.number().int().min(0).max(9999).default(0),
  locales: z.object({
    ar: institutionExpertLocalizedSchema,
    en: institutionExpertLocalizedSchema,
  }),
});
...
const researchTeamMemberSchema = z.object({
  ...
  institutionExpertId: z.string().optional().nullable(),
});
...
export const researchInputSchema = z.object({
  ...
  data.teamMembers.forEach((member, index) => {
    if (member.isCorresponding && !member.institutionExpertId) {
      ctx.addIssue({
        code: "custom",
        path: ["teamMembers", index, "institutionExpertId"],
        message: "Institution expert is required for corresponding author",
      });
    }
  });
});
