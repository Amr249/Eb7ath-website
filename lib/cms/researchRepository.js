  ...
  for (let i = 0; i < teamMembers.length; i++) {
    ...
    await sql()`
      INSERT INTO research_team_members (
        ...
        affiliation_en, affiliation_ar, email, is_corresponding, institution_expert_id
      )
      VALUES (
        ${randomUUID()},
        ${researchId},
        ...
        ${member.institutionExpertId || null}
      )
    `;
  }
}
...
function mapTeamMember(row) {
  return {
    ...
    institutionExpertId: row.institution_expert_id || "",
  };
}
...
export async function getResearchAdmin(id) {
  ...
  const teamRows = await sql()`
    SELECT
      id, name_en, name_ar, affiliation_en, affiliation_ar, email, is_corresponding,
      institution_expert_id
  ...
  return {
    ...
    correspondingAuthor: corresponding
      ? {
          ...
          affiliation: { en: corresponding.affiliation_en, ar: corresponding.affiliation_ar },
          profileUrl: corresponding.researchgate_url || undefined,
        }
      : undefined,
    researchers: teamRows.map((member) => ({
      ...
      email: member.is_corresponding ? undefined : member.email || undefined,
      isCorresponding: member.is_corresponding,
      profileUrl: member.is_corresponding ? member.researchgate_url || undefined : undefined,
    })),
  };
}
...
export async function getPublishedResearchBySlug(slug) {
  ...
  const teamRows = await sql()`
    SELECT
      t.name_en, t.name_ar, t.affiliation_en, t.affiliation_ar, t.email, t.is_corresponding,
      ie.researchgate_url
    FROM research_team_members t
    LEFT JOIN institution_experts ie ON ie.id = t.institution_expert_id
    WHERE t.research_id = ${rows[0].id}
